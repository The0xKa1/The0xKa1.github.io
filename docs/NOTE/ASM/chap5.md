...
| 指令 | 全称 | 功能描述 | 对原位的影响 |
| --- | --- | --- | --- |
| **BT** | **B**it **T**est | 测试指定位，将其值存入 **CF**。 | **无影响**（只读） |
| **BTS** | **B**it **T**est and **S**et | 测试并将位值存入 **CF**，随后将该位置 **1**。 | **置 1** |
| **BTR** | **B**it **T**est and **R**eset | 测试并将位值存入 **CF**，随后将该位置 **0**。 | **清 0** |
| **BTC** | **B**it **T**est and **C**omplement | 测试并将位值存入 **CF**，随后将该位**翻转**。 | **取反** (0$\to\to$0) |
